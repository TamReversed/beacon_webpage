...
app.get('/api/footer-links', (req, res) => {
  try {
    const links = listFooterLinks({ visibleOnly: true });
    res.json({ footerLinks: links });
  } catch (error) {
...
app.get('/api/admin/footer-links', requireLogin, requireAdmin, (req, res) => {
  try {
    const links = listFooterLinks({ visibleOnly: false });
    res.json({ footerLinks: links });
  } catch (error) {
    console.error('Error fetching footer links:', error);
    res.status(500).json({ error: 'Failed to fetch footer links' });
...
app.post('/api/admin/footer-links', requireLogin, requireAdmin, (req, res) => {
  try {
    const { column_name, label, url, sort_order, is_note } = req.body;
    if (!column_name || !label || !url) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    const id = createFooterLink(column_name, label, url, sort_order || 0, is_note || 0);
...
app.put('/api/admin/footer-links/:id', requireLogin, requireAdmin, (req, res) => {
  try {
    const { id } = req.params;
    const { column_name, label, url, sort_order, is_note, visible } = req.body;
    const success = updateFooterLink(id, { column_name, label, url, sort_order, is_note, visible });
    if (success) {
...
app.delete('/api/admin/footer-links/:id', requireLogin, requireAdmin, (req, res) => {
  try {
    const { id } = req.params;
    const success = deleteFooterLink(id);
    if (success) {
      res.json({ success: true });
    } else {
      res.status(404).json({ error: 'Footer link not found' });
