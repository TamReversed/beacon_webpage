...
(function () {
  ...
  function loadFooterLinks() {
    fetch('/api/admin/footer-links', { credentials: 'include' })
      .then(function (r) { return r.json(); })
      .then(function (data) {
        var tbody = document.getElementById('tbody-footer-links');
        if (!tbody) return;
        tbody.innerHTML = '';
        (data.footerLinks || []).forEach(function (link) {
          var tr = document.createElement('tr');
          tr.innerHTML = 
            '<td><input class="footer-column" data-id="' + link.id + '" value="' + escapeHtml(link.column_name) + '" /></td>' +
            '<td><input class="footer-label" data-id="' + link.id + '" value="' + escapeHtml(link.label) + '" /></td>' +
            '<td><input class="footer-url" data-id="' + link.id + '" value="' + escapeHtml(link.url) + '" /></td>' +
            '<td><input type="number" class="footer-sort" data-id="' + link.id + '" value="' + link.sort_order + '" style="width: 60px;" /></td>' +
            '<td><input type="checkbox" class="footer-is-note" data-id="' + link.id + '" ' + (link.is_note ? 'checked' : '') + ' /></td>' +
            '<td><button class="admin-status-pill admin-status-pill--' + (link.visible ? 'visible' : 'hidden') + '" data-id="' + link.id + '" data-visible="' + link.visible + '">' + (link.visible ? 'Showing' : 'Hidden') + '</button></td>' +
            '<td><button class="btn btn-sm btn-save-footer" data-id="' + link.id + '">Save</button> <button class="btn btn-sm btn-ghost btn-delete-footer" data-id="' + link.id + '">Delete</button></td>';
          tbody.appendChild(tr);
        });
      })
      .catch(function (err) {
        console.error('Error loading footer links:', err);
        showAdminMessage('Failed to load footer links', true);
      });
  }

  var formAddFooterLink = document.getElementById('form-add-footer-link');
  if (formAddFooterLink) {
    formAddFooterLink.addEventListener('submit', function (e) {
      e.preventDefault();
      ...
      var data = {
        ...
        is_note: formData.get('is_note') ? 1 : 0,
      };
      fetch('/api/admin/footer-links', {
        ...
        .then(function (r) {
          if (r.ok) {
            e.target.reset();
            loadFooterLinks();
            showAdminMessage('Footer link added', false);
          } else {
            r.json().then(function (d) { showAdminMessage(d.error || 'Failed to add link', true); }).catch(function () { showAdminMessage('Failed to add link', true); });
          }
        })
        .catch(function () { showAdminMessage('Failed to add link', true); });
    });
  }

  var tbodyFooterLinks = document.getElementById('tbody-footer-links');
  if (tbodyFooterLinks) {
    tbodyFooterLinks.addEventListener('click', function (e) {
      var id = e.target.getAttribute('data-id');
      if (!id) return;
        ...
        fetch('/api/admin/footer-links/' + id, {
          ...
          .then(function (r) {
            if (r.ok) {
              ...
              e.target.textContent = next ? 'Showing' : 'Hidden';
              e.target.className = 'admin-status-pill admin-status-pill--' + (next ? 'visible' : 'hidden');
              showAdminMessage('Visibility updated', false);
            } else {
              showAdminMessage('Update failed', true);
            }
          })
          .catch(function () { showAdminMessage('Update failed', true); });
        return;
      ...
      if (e.target.classList.contains('btn-save-footer')) {
        var row = e.target.closest('tr');
        var column_name = row.querySelector('.footer-column').value;
        var label = row.querySelector('.footer-label').value;
        var url = row.querySelector('.footer-url').value;
        var sort_order = parseInt(row.querySelector('.footer-sort').value) || 0;
        var is_note = row.querySelector('.footer-is-note').checked ? 1 : 0;

        fetch('/api/admin/footer-links/' + id, {
          method: 'PUT',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ column_name, label, url, sort_order, is_note }),
        })
          .then(function (r) {
            if (r.ok) {
              e.target.textContent = 'Saved';
              setTimeout(function () { e.target.textContent = 'Save'; }, 1500);
              showAdminMessage('Footer link updated', false);
            } else {
              r.json().then(function (d) { showAdminMessage(d.error || 'Update failed', true); }).catch(function () { showAdminMessage('Update failed', true); });
            }
          })
      ...
      if (e.target.classList.contains('btn-delete-footer')) {
        if (!confirm('Delete this footer link?')) return;
        fetch('/api/admin/footer-links/' + id, { method: 'DELETE', credentials: 'include' })
          .then(function (r) {
            if (r.ok) {
              loadFooterLinks();
              showAdminMessage('Footer link deleted', false);
            } else {
